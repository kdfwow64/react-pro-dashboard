...
export default class StatefulToolTip extends React.Component<any, any> {
    ...
    render() {
        ...
        let width = imageWidth + (2 * 20); //20 is the padding in the outer `style`
        const style = {
            style: {
                background: '#fff',
                padding: 20,
        ...
        return (
            <span>
                ...
                <Tooltip {...props} active={this.state.tooltipVisible} parent={"#" + id} tooltipTimeout={500} style={style}>
                    <p style={textStyle} dangerouslySetInnerHTML={{ __html: text }}></p>
                    <img hidden={imgSrc === undefined || imgSrc === null || imgSrc === ''} src={`img/tooltippictures/${imgSrc}`} style={{ width: imageWidth, height: 'auto' }} />
                </Tooltip>
            </span>
        );
    }
