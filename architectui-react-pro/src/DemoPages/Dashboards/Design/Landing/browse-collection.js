...
class BrowseCollection extends React.Component  {
     constructor(props) {
        ...
        this.state = {
            ...
            footerBGColor: this.props.callType !== 'droppable' ? this.props.BrowsefooterBGColor : this.props.settings.footerBGColor,
    ...
    componentWillMount() {
        if (this.state.collectionType && this.state.selectedProductName){
            axios.get(`${API_ROOT}/api/v2/collection/products/` + this.state.collectionType + `/` + this.state.selectedProductName)
            .then(res1 => {
    ...
    render() {
        return (
            <div className="HoverEffectDragDrop" id="SixthDropContainer" style={{ overflowX: 'scroll' }}>
             {
                this.state.CollectionsProductsData.length !== 0 &&
                this.state.CollectionsProductsData.map((item , index ) => {
                    return (<div key={index}>
                        <div className="SixthGridContainer" style={{ backgroundColor: this.state.imageSpacingColor }}>
                            <div className="SixthGridLeftTopPicture" style={{ border: `1px solid ${this.state.imageSpacingColor}` }}>
            ...
            <div className="SixthGridHeaderDetails" style={{ color: this.state.titleColor, backgroundColor: this.state.footerBGColor }}>
                <span id="SixthGridHeaderDetailsTitle" >{this.state.title}</span>&nbsp;
                <span id="SixthGridHeaderIcon">
                    <i style={{ color: this.state.iconColor }} className={this.state.DropDownGetIconheader}></i>
                </span>
            </div>
        </div>
        );
